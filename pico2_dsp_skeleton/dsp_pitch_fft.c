 ...
 * Phillip McLeod, heavily accelerated by FFT Cross-Correlation.
 */
...
#else
#error "BUFFER_SIZE too large for CMSIS-DSP FFT"
#endif

// Synchronous copy allows history size to perfectly match the buffer size.
#define CIRCULAR_HISTORY_SIZE BUFFER_SIZE

#define TAU_MIN 8
#define TAU_MAX MAX_TAU
...
// FFT Specific Buffers
static arm_rfft_fast_instance_f32 fft_inst;
static float32_t fft_buf_a[FFT_SIZE];
static float32_t fft_buf_b[FFT_SIZE];
static float32_t fft_out_a[FFT_SIZE];
static float32_t fft_out_b[FFT_SIZE];
static float32_t fft_mult[FFT_SIZE];
static float32_t acf_result[FFT_SIZE];
...
static void compute_nsdf(float32_t *window)
{
  ...
  // Zero-pad Buffer A (The Fixed Window)
  memcpy(fft_buf_a, window, WINDOW_SIZE * sizeof(float32_t));
  memset(fft_buf_a + WINDOW_SIZE, 0, (FFT_SIZE - WINDOW_SIZE) * sizeof(float32_t));

  // Zero-pad Buffer B (The Search Space)
  memcpy(fft_buf_b, window, BUFFER_SIZE * sizeof(float32_t));
  memset(fft_buf_b + BUFFER_SIZE, 0, (FFT_SIZE - BUFFER_SIZE) * sizeof(float32_t));

  // Execute Forward FFTs
  arm_rfft_fast_f32(&fft_inst, fft_buf_a, fft_out_a, 0);
  arm_rfft_fast_f32(&fft_inst, fft_buf_b, fft_out_b, 0);

  // Complex Conjugate Multiplication
  // CMSIS-DSP packs DC and Nyquist into index 0 and 1 (purely real)
  fft_mult[0] = fft_out_a[0] * fft_out_b[0];
  fft_mult[1] = fft_out_a[1] * fft_out_b[1];

  for (int i = 2; i < FFT_SIZE; i += 2) {
    float32_t are = fft_out_a[i];
    float32_t aim = fft_out_a[i+1];
    float32_t bre = fft_out_b[i];
    float32_t bim = fft_out_b[i+1];
    
    // Conj(A) * B
    // (are - j*aim) * (bre + j*bim)
    fft_mult[i]   = (are * bre) + (aim * bim); // Re
    fft_mult[i+1] = (are * bim) - (aim * bre); // Im
  }

  // Execute Inverse FFT
  arm_rfft_fast_f32(&fft_inst, fft_mult, acf_result, 1);
...
static int mpm_peak_pick(void)
{
  ...
  // Set the relative threshold. McLeod's recommended k ranges from 0.8 to 1.0.
  float32_t threshold = max_peak * 0.93f;
    
  // Scan from the beginning and pick the first local maximum that crosses the threshold
  for (int tau = TAU_MIN; tau < TAU_MAX - 1; tau++) {
...
static void core1_main(void)
{
  ...
  while (1) {
    ...
    // Apply multiframe median filter to reduce instantaneous
